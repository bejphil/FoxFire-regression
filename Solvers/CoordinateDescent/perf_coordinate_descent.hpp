...
void RunCDPerfs() {
    ...
    for ( unsigned int k = 200; k <= 1000; k+= 200 ) {
        ...
        T standard_result = PerfCD< T, foxfire::LazyCoordinateDescent<T, foxfire::internal::Solver<T> > >( X, Y, W_0, 10 );
        standard_results.push_back( standard_result );

        std::cout << "Testing Screened CD." << std::endl;
        T screened_result = PerfCD< T, foxfire::LazyCoordinateDescent<T,foxfire::internal::ScreeningSolver<T>> >( X, Y, W_0, 10 );
