...
void RunFISTATests() {
    ...
    for ( unsigned int k = 1000; k <= 5000; k+= 1000 ) {
        ...
        Eigen::Matrix< T, Eigen::Dynamic, 1 > cpu_results = TestFISTA< T, foxfire::FISTA<T> >( X, Y, W_0, 10 );
        Eigen::Matrix< T, Eigen::Dynamic, 1 > gpu_results = TestFISTA< T, foxfire::CL_FISTA<T> >( X, Y, W_0, 10 );
